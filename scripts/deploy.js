...
async function main() {
    ...
    const marketplaceInit = await deploy("MarketplaceInit", deployer);
    const marketplaceProxy = await deploy("MarketplaceProxy", deployer, marketplaceInit.address);

    const marketplaceInitializable = await hre.ethers.getContractAt("IInitializable", marketplaceProxy.address);
    const marketplaceUpgrade = await hre.ethers.getContractAt("IUpgrade", marketplaceProxy.address);
    const marketplaceUpgradable = await hre.ethers.getContractAt("IUpgradable", marketplaceProxy.address);
    const marketplace = await hre.ethers.getContractAt("IMarketplace", marketplaceProxy.address);
    console.log("Marketplace: ", marketplace.address);

    await marketplaceInitializable.initialize(coder.encode(
        ["address", "address", "address", "address"],
        [deployer.address, upgradesRegistry.address, nftInit.address, inventoryInit.address]
    ));

    let tx = await marketplace.deployToken(
        "Token name",
        "TKN",
        "uri",
        deployer.address,
    ...
    await nftLootboxInterface.setLootAddress(erc721.address);
